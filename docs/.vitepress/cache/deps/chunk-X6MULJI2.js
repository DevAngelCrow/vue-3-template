...
var BadgeDirective = BaseBadgeDirective.extend("badge", {
  mounted: function mounted(el, binding) {
    ...
    var id = s("pv_id") + "_badge";
    var badge = U("span", _defineProperty(_defineProperty({
      id,
    ...
    for (var modifier in binding.modifiers) {
      !this.isUnstyled() && W(badge, "p-badge-" + modifier);
    }
    if (binding.value != null) {
      ...
      if (String(el.$_badgeValue).length === 1 && !this.isUnstyled()) {
        !this.isUnstyled() && W(badge, "p-badge-circle");
      }
    } else {
      !this.isUnstyled() && W(badge, "p-badge-dot");
    }
    el.setAttribute("data-pd-badge", true);
    !this.isUnstyled() && W(el, "p-overlay-badge");
    ...
    if (binding.oldValue !== binding.value) {
      ...
      if (!this.isUnstyled()) {
        if (el.$_badgeValue) {
          ...
          if (el.$_badgeValue.length === 1) W(badge, "p-badge-circle");
...
//# sourceMappingURL=chunk-X6MULJI2.js.map

    ...
    <section id="departments_content" class="w-full flex flex-row flex-wrap gap-5">
      <div class="w-full flex flex-row gap-3 flex-wrap">
        ...
        <div id="inputs" class="flex rounded-lg border-2 border-primary py-0.5 px-0.5 gap-3 flex-wrap grow lg:grow-0">
          <AppInputText label="Buscar" class="min-w-auto w-full grow shrink-0 sm:w-[50%] md:w-[45] lg:w-83.75"
            v-model="filter.filter_name" @update:modelValue="validateAlphaInput(filter.filter_name)"
            v-debounce:700.keydown.enter="() => findDepartment(filter)" />
          <AppSelect class="w-full sm:w-[20%] lg:w-auto min-w-0 grow shrink-0" :options="countries" option-label="name" label="País"
            v-model="filter.id_country" optionValue="id" />
          ...
          <Button class="shrink-0 grow rounded-md"
            v-debounce:700.click="() => findDepartment(filter)">Buscar</Button>

        ...
        <template #header>
          <div class="flex flex-col items-center pt-8 pb-2 px-8 gap-3">
            ...
            <div class="text-center">
              <h1 class="text-2xl font-semibold text-white tracking-tight">Crear cuenta</h1>
              <p class="text-sm text-zinc-400 mt-1">Completa los datos para registrarte</p>
            </div>
          </div>
        </template>
          ...
          <form class="flex flex-col gap-5 px-8 pb-8 pt-4" @submit.prevent="onSubmit">
            ...
            <div class="flex flex-col gap-4">
              <AppInputText placeholder="Correo electrónico" v-model="email" show-icon
                ...
                label="Correo electrónico" class="w-full" :disabled="isLoading"
                @update:modelValue="validationInputEmail(email, 'email')" />
              <AppInputText placeholder="Contraseña" v-model="password" show-icon prepend-inner-icon="pi pi-lock"
                :error-messages="errors.password" v-bind="passwordAttrs" autocomplete="new-password" type="password"
                label="Contraseña" class="w-full" :disabled="isLoading"
                @update:modelValue="validationInputPassword(password, 'password')" />
            </div>

            <!-- Submit -->
            <Button type="submit" :loading="isLoading" :disabled="isLoading" class="w-full mt-1" size="large">
              <template #default>
            ...
            <p class="text-center text-sm text-zinc-400">
              ...
              <a href="/login" class="text-primary-400 hover:text-primary-300 transition-colors ml-1">
                Inicia sesión
              </a>
            </p>
          </form>
